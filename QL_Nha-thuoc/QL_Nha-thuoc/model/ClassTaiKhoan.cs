...
namespace QL_Nha_thuoc.model
{
    ...
    public class ClassTaiKhoan
    {
        ...
        public ClassTaiKhoan(string tenTK, string matKhau, string maNV,string vaiTro, NhanVien nv)
        {
            TenTaiKhoan = tenTK;
            MatKhau = matKhau;
            MaNhanVien = maNV;
            ...
            using (SqlConnection conn = DBHelper.GetConnection())
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        return new ClassTaiKhoan(tenTK, matKhau, maNV,vaiTro, nhanVien);
                    }
                }
            }
            ...
            using (SqlConnection conn = DBHelper.GetConnection())
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        string vaiTro = reader["TEN_VAI_TRO"].ToString();
                        var nhanVien = new NhanVien(maNV, tenNV, vaiTro);
                        var taiKhoan = new ClassTaiKhoan(tenTK, matKhau, maNV, vaiTro, nhanVien);
